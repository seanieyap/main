...
You can also list all _slots_ in the planner as well. +
Format: `list all`

[TIP]
Keywords are not case-sensitive. (e.g. _CS2113T_ is the same as _cs2113t_)
...
* `list t/Lecture` +
List all _slots_ that contain the tag `Lecture`.
...
An empty command history is initialised when _PlanMySem_ is started. It will be
cleared upon exiting _PlanMySem_.
